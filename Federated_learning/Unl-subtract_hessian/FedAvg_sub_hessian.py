...
class PoisonedDataset(Dataset):
    ...
    def __init__(self, dataset, base_label, trigger_label, portion, mode="train", device=torch.device("cuda"), dataname="mnist"):
        ...
        if self.dataname=="MNIST":
            self.data, self.targets = self.add_trigger(self.p_reshape(dataset.data, dataname), dataset.targets, base_label, trigger_label, portion, mode)
        elif self.dataname=="CIFAR10":
            self.data, self.targets = self.add_trigger(self.p_reshape(dataset, dataname), dataset.targets,
                                                       base_label, trigger_label, portion, mode)
    ...
    def p_reshape(self, dataset, dataname="CIFAR10"):
        if dataname == "MNIST":
            temp_img = dataset.data.reshape(len(dataset.data),1,28,28)
        elif dataname == "CIFAR10":
...
class LocalUpdate(object):
    def __init__(self, args, dataset=None, idxs=None, need_poison=None, poison_data=None,poison_targets=None):
        self.args = args
        ...
        self.remaining_set, self.erasing_set = torch.utils.data.random_split(self.train_splite, [remaining_size, erasing_size])
    ...
    def unlearn_Bayes(self, net, net_temp, idx, args):
        # train and update
        #self.remaining_loader = DataLoader(self.remaining_set, batch_size=self.args.local_bs, shuffle=True)
        ...
        for epoch in range(self.args.local_ep):
            step_start = epoch * len(self.ldr_train)
            net, optimizer = LocalUpdate.unlearning_nips(net, optimizer, self.erased_loader, remaining_loader, self.loss_func, reconstruction_function, net_temp, args ,epoch, idx)
            # net.eval()
        return net.state_dict()
    ...
    @staticmethod
    def unlearning_nips(net, optimizer_nips, dataloader_erase, remaining_loader, loss_fn, reconstruction_function, net_temp, args, epoch, idx):
        logs = defaultdict(list)
        mu_list = []
        sigma_list = []
        init_epoch = 0
        ...
        for   (x, y), (x2,y2) in   zip(dataloader_erase, remaining_loader)  :
            ...
            loss = args.beta * KLD_mean + H_p_q - log_z / e_log_py +H_p_q2
            ...
            step=step+1
            if step % len(dataloader_erase) % 600 == 0 and epoch==2:
                print(f'[{epoch}/{init_epoch + args.num_epochs}:{step % len(dataloader_erase):3d}] '
                      + ', '.join([f'{k} {v:.3f}' for k, v in metrics.items()]))

        net.eval()
        return net, optimizer_nips
    ...
    def retraining_kld(net, optimizer_retrain, dataloader_remain, loss_fn, reconstruction_function, net_org, net_unl, args, epoch, idx, KL_fr, KL_nipsr):
        ...
        for step, (x, y) in enumerate(dataloader_remain, start=step_start):
            ...
            loss_r = args.beta * KLD_mean + H_p_q #+ BCE / (args.local_bs * 28 * 28)
...
def FL_train(net_glob, args, dataset_train, dataset_test, dict_users, idxs_local_dict, train_type="train"):
    net_glob.train()
...
def FL_retrain(net_glob, net_org, net_unl, args, dataset_train, dataset_test, dict_users, idxs_local_dict, retrain_epoch, train_type="retrain"):
    net_glob.train()
...
def FL_unlearn_train(net_glob, net_temp, args, dataset_train, dataset_test, dict_users, idxs_local_dict, train_type="train"):
    net_glob.train()
    w_glob = net_glob.state_dict()
    acc_test = []
    ...
    for iter in range(args.epochs):
        ...
        for idx in idxs_users:
            ...
            local = idxs_local_dict[idx]
            if train_type=='unlearn':
                net_local_w = local.unlearn_Bayes(copy.deepcopy(net_glob).to(args.device), net_temp, idx, args)
            glob_w = net_glob.state_dict()
            new_lcoal_grad = net_glob.state_dict()
            for k in glob_w.keys():
                new_lcoal_grad[k] = glob_w[k] - net_local_w[k]
...
def clean_FL(args):
    ...
    poison_perm = np.random.permutation(args.num_users)[0: int(args.num_users * args.poison_portion)]
    ...
    poison_data, poison_targets = create_backdoor_train_dataset(dataname=args.dataset, train_data=dataset_train, base_label=1,
                                                  trigger_label=7, posioned_portion=args.poison_portion,
    ...
    print("start train")
    net_glob = FL_train(net_glob, args, dataset_train, dataset_test, dict_users, idxs_local_dict, train_type='train')
    ...
    #unlearn_nips = FL_unlearn_train(unlearn_nips, net_glob, args, dataset_train, dataset_test, dict_users, idxs_local_dict, train_type='unlearn')

    unlearn_nips = unlearning_net_global(unlearn_nips, idxs_local_dict, args, dataset_test, erased_perm)
    ...
    print("start retrain")
    retrian_net, KL_fr, KL_nipsr = FL_retrain(retrian_net, net_glob, unlearn_nips, args, dataset_train, dataset_test, dict_users, idxs_local_dict, retrain_epoch= args.epochs, train_type='retrain')
...
if __name__ == '__main__':
    ...
    args.beta = 0.01
    args.lr = 0.001
    args.erased_size = 4000
    args.poison_portion = 0
    args.erased_portion = 0.1
    clean_FL(args)
