    ...
    def unlearning_nips(net, optimizer_nips, dataloader_erase, remaining_loader, loss_fn, reconstruction_function, net_temp, args, epoch, idx, train_type):
        ...
        for   (x, y), (x2,y2) in   zip(dataloader_erase, remaining_loader)  :
            ...
            if train_type == 'unlearn_nips':
                loss = unl_KLD_lr * KLD_mean - unlearn_learning_rate * H_p_q - log_z * e_log_py#- BCE + kl_f_e  #1/H_p_q   #- log_z * e_log_py #e_log_py #args.beta * KLD_mean - H_p_q #- BCE / (args.local_bs * 28 * 28) #- H_p_q #- log_z / e_log_py #+ H_p_q2 - H_p_q #
                ...
                loss = unl_KLD_lr * KLD_mean  - unlearn_learning_rate * H_p_q - log_z * e_log_py + self_sharing_rate * (args.beta * KLD_mean2 + H_p_q2) # args.beta * KLD_mean - H_p_q + args.beta * KLD_mean2  + H_p_q2 #- log_z / e_log_py #-   # H_p_q + args.beta * KLD_mean2
...
def FL_unlearn_train(net_glob, net_temp, args, dataset_train, dataset_test, dict_users, idxs_local_dict, erased_perm,poison_testset, train_type="train"):
    ...
    for iter in range(args.local_ep+10):
        # dict_usersZ = []
        idxs_users = range(args.num_users)
        w_locals = []
        grad_locals = []
