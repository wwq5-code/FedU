...
def init_vibi(dataset):
    ...
    if dataset == 'MNIST':
        ...
        approximator = LinearModel(n_feature=8*8*8, n_output=10)
        explainer = resnet18(3,  8*8*8*2)  # resnet18(1, 49*2)
        forgetter = LinearModel(n_feature=8*8*8, n_output=3 * 96 * 96)
        lr = 3e-4
...
class LocalUpdate(object):
    def __init__(self, args, dataset=None, idxs=None, need_poison=None, poison_data=None,poison_targets=None, erased_perm=None, idx=None):
        ...
        if need_poison:
            if self.args.dataset=='MNIST':
                ...
                data_reshape = self.train_splite.data.reshape(len(self.train_splite.data), 3, 96, 96)
    ...
    def unlearn_Bayes(self, net, net_temp, idx, args, train_type):
        ...
        batch_acc = []
        batch_b_acc = []
        for epoch in range(self.args.local_ep+10):
            step_start = epoch * len(self.ldr_train)
            net, optimizer, acc_list, unlearned_acc_list, batch_acc, batch_b_acc = LocalUpdate.unlearning_nips(net, optimizer, self.erased_loader, self.remaining_loader2, self.loss_func, reconstruction_function, net_temp, args ,epoch, idx, train_type, batch_acc, batch_b_acc)
    ...
    for iter in range(args.epochs):
        ...
        for idx in idxs_users:
            if idx not in erased_perm:
                ...
                print(idx, "start idx unlearning")
                local = idxs_local_dict[idx]
...
def clean_FL(args):
    ...
    elif args.model == 'z_linear' and args.dataset == 'STL10':
    ...
    print("unlearning running time of one epoch", end - start)
...
if __name__ == '__main__':
    ...
    # args.unlearn_learning_rate = 0.000025
    # args.self_sharing_rate = 0.0008
    # args.unlearn_ykl_r = 0.001
